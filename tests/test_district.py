...
@pytest.mark.parametrize("cant_tributes",
                         [4, 5, 6])  # test parametrizado, la funcion se ejecuta tres veces, con 4, con 5 y con 6.
def test_set_config_valid_cant_tributes(cant_tributes):
    district = District()
    district.set_config(50, 6, 8, 2, cant_tributes)
    assert len(
        district.tributes) == cant_tributes  # Por defecto la cantidad de tributos es 4, pero podrian agregarse 1 o 2 mas
